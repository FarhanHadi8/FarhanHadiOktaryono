...
form.addEventListener("submit", (e) => {
  ...
  // Kirim data ke scriptURL (fire-and-forget - dengan mode "no-cors" kita
  // tidak bisa membaca isi response-nya, jadi tidak perlu menunggu balasan
  // dari server yang waktunya bisa lama & tidak konsisten)
  fetch(scriptURL, { method: "POST", body: new FormData(form), mode: "no-cors" }).catch((error) => {
    console.error("Error!", error.message);
  });

  // Sembunyikan loading & tampilkan alert langsung setelah request dikirim
  if (loading) {
